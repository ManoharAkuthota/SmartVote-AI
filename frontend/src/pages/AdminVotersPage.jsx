...
import { Search, CheckCircle, XCircle, Lock, Unlock, Eye, RefreshCw, X, ShieldAlert, LogOut } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex items-center space-x-2 mb-6 pb-2 border-b border-slate-200 dark:border-slate-800 overflow-x-auto">
        ...
        <Link
          to="/admin/voters"
          className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-500/30 transition"
        >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-300 border border-amber-500/30 uppercase">
            National Electoral Roll (ECI)
          </span>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white mt-1">Voter Enrollment & Verification</h1>
        </div>

        <button
          onClick={logout}
          className="px-3.5 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 active:bg-rose-500/30 border border-rose-500/30 text-rose-600 dark:text-rose-400 text-xs font-bold transition flex items-center space-x-1.5 shadow-sm cursor-pointer self-start sm:self-auto"
