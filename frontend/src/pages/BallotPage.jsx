...
export default function BallotPage() {
  ...
  const fetchElection = async () => {
    ...
    try {
      ...
      setErrorMsg('Failed to load ballot details.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCastVote = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        setErrorMsg(res.data?.message || 'Voting failed.');
      }
    } catch (err) {
      setErrorMsg(err.response?.data?.message || 'Vote rejected: You may have already voted or election is closed.');
      setShowConfirmModal(false);
    } finally {
      setIsCasting(false);
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-slate-400">
        ...
        <p className="text-xs">Accessing cryptographic ballot chamber...</p>
      </div>
    );
  }
  ...
  if (!election) {
    return (
      <div className="max-w-xl mx-auto py-16 px-4 text-center">
        ...
        <Link to="/elections" className="text-xs text-cyan-400 mt-2 block">
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <Link
        to="/dashboard"
        className="inline-flex items-center space-x-1.5 text-xs text-slate-400 hover:text-cyan-400 mb-6 transition"
      >
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/90 border border-cyan-500/30 shadow-2xl relative overflow-hidden mb-8">
      ...
      {election.hasVoted && (
        <div className="mb-6 p-4 rounded-2xl bg-emerald-950/80 border border-emerald-500/50 flex items-center space-x-3 text-emerald-300 text-xs">
          ...
          <div>
            <strong className="block text-sm">Your Ballot is Already Sealed</strong>
            You have already cast your single permitted vote for this election. The ledger is immutable.
          </div>
        </div>
      )}
      ...
      {/* Submit Button Bar */}
      {!election.hasVoted && (
        <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 sticky bottom-4 shadow-2xl backdrop-blur-xl">
          <div>
            <span className="text-xs text-slate-400">Current Ballot Selection:</span>
            <div className="text-sm font-bold text-cyan-400">
              {selectedCandidate ? `${selectedCandidate.fullName} (${selectedCandidate.partyName})` : 'No candidate selected'}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-gradient-to-r from-cyan-400 via-purple-500 to-emerald-400 text-slate-950 font-black text-xs uppercase tracking-wider shadow-neon-cyan hover:opacity-95 disabled:opacity-40 transition"
          >
            Review & Seal Vote
          </button>
        </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl animate-fade-in">
          <div className="w-full max-w-md p-6 sm:p-8 rounded-3xl bg-slate-900 border-2 border-amber-500/40 shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-center text-white">Immutable Vote Confirmation</h3>
            <p className="text-xs text-center text-slate-300 mt-2 leading-relaxed">
              You are about to cast your official ballot for:
            </p>

            <div className="my-4 p-4 rounded-xl bg-slate-950 border border-slate-800 text-center">
              <div className="text-base font-black text-cyan-400">{selectedCandidate?.fullName}</div>
              <div className="text-xs text-slate-400 mt-0.5">{selectedCandidate?.partyName} {selectedCandidate?.partySymbol}</div>
            </div>

            <div className="p-3 rounded-xl bg-amber-950/40 border border-amber-500/30 text-[11px] text-amber-200 mb-6 leading-relaxed">
              <strong>CRITICAL NOTICE:</strong> Once submitted, your vote will be signed with a cryptographic SHA-256 seal and locked permanently. You cannot change your choice under any circumstance.
            </div>
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                disabled={isCasting}
                className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-400 to-cyan-400 text-slate-950 font-black text-xs uppercase tracking-wider shadow-neon-green hover:opacity-95 disabled:opacity-50 flex items-center justify-center space-x-2 transition"
              >
                {isCasting ? (
                  <>
