...
import { ShieldCheck, Download, RefreshCw, AlertTriangle, CheckCircle2, Lock, Smartphone, LogOut } from 'lucide-react';
...
export default function AdminAuditLogsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex items-center space-x-2 mb-6 pb-2 border-b border-slate-200 dark:border-slate-800 overflow-x-auto">
        <Link
          to="/admin"
          className="px-3.5 py-1.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition"
        >
          Overview
        </Link>
        <Link
        ...
        <Link
          to="/admin/audit"
          className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/30 transition"
        >
          Security & Audit Logs
        </Link>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-purple-500/20 text-purple-600 dark:text-purple-300 border border-purple-500/30 uppercase">
            Cryptographic Integrity Audit (Article 324)
          </span>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white mt-1">Electoral Security & Access Telemetry</h1>
        </div>

        <div className="flex items-center space-x-2">
          <button
            onClick={handleExportCsv}
            className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-amber-500/40 hover:border-amber-500 text-amber-700 dark:text-amber-300 text-xs font-bold shadow-md flex items-center space-x-2 transition cursor-pointer"
          >
          ...
          <button
            onClick={logout}
            className="px-3.5 py-2.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 active:bg-rose-500/30 border border-rose-500/30 text-rose-600 dark:text-rose-400 text-xs font-bold transition flex items-center space-x-1.5 shadow-sm cursor-pointer"
